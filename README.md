...
- **[新] 行情自洽核对：`close[t]` 与 `pre_close[t+1]` 对不上**
  ...
  **未修复的 `close` 错值（4 行，全部在同一天、全部沪市）**：

  | 代码 | 日期 | 库内 close | 同花顺 close | 次日 pre_close |
  |---|---|---|---|---|
  | 600822.SH | 2016-12-01 | 16.71 | 16.70 | 16.70 |
  | 603101.SH | 2016-12-01 | 28.22 | 28.21 | 28.21 |
  | 603117.SH | 2016-12-01 | 17.59 | 17.58 | 17.58 |
  | 603737.SH | 2016-12-01 | 82.55 | 82.51 | 82.51 |

  同花顺收盘价与 baostock **自己的**次日 `pre_close` 一致，共同指向 baostock 的
  `close` 有误（2 比 1）。暂不修的原因：日线没有 gbbq 那样的「定点修正」机制，
  裸 UPDATE 会被任何一次该日期区间的重新导入静默还原；而为 4 行 0.01~0.04 元的
  误差新建一套机制，代价与收益不相称。**要修就得连机制一起做。**
