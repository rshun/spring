...
- tdx  
   ...
   - **000850.SZ这只股票在2021-03-26股本资料有问题，总股本返回是1850(1850万股)，但实际应该是18500(1.85亿股)**
...
- 补齐pb,pe
- **[新] STOCK_DAILY.close 个别日期存在错值，需单独立项核对**

  2026-09-13 在复权因子恒等式核对（`tools/checks/adjust_invariant.py`）中作为副产品
  发现：某些交易日的 `close` 与次日的 `pre_close` 对不上，用同花顺收盘价仲裁后，
  **绝大多数是 `close` 错而非 `pre_close` 错**。

  | 代码 | 前一交易日 | 库内 close | 同花顺 close | 次日 pre_close | 判定 |
  |---|---|---|---|---|---|
  | 688065.SH | 2023-06-23 | 55.02 | 55.53 | 55.53 | close 错 |
  | 600822.SH | 2016-12-01 | 16.71 | 16.70 | 16.70 | close 错 |
  | 603101.SH | 2016-12-01 | 28.22 | 28.21 | 28.21 | close 错 |
  | 603117.SH | 2016-12-01 | 17.59 | 17.58 | 17.58 | close 错 |
  | 603737.SH | 2016-12-01 | 82.55 | 82.51 | 82.51 | close 错 |
  | 600654.SH | 2022-12-22 | 2.86 | 2.86 | 2.85 | pre_close 错（唯一反向） |

  特征：误差 0.01~0.5 元量级；同一天多只沪市股票同时出现（`2016-12-02` 四只、
  `2013-05-21` 三只），像数据源批次问题而非公司行为。

  影响：这类错值会污染一切基于 `close` 的计算（收益率、均线、换手率分母等），
  且**不属于复权范畴**——目前只是被复权核对顺带撞见的。已知 2011 年以来至少 12 处。

  建议：新增一个独立的行情自洽核对项，判据为
  `close[t] == pre_close[t+1]`（非除权日）；命中后用同花顺 dump 仲裁哪一侧错。
  注意只有 `bstock` 带真实 `preclose`，`lday`/`tdx` 是 `LAG(close)` 现算的，对这类
  股票该判据恒成立、查不出问题（见「已知问题 > lday」）。
