...
- **[新] 行情自洽核对：`close[t]` 与 `pre_close[t+1]` 对不上**
  ...
  **`close` 源数据错值（4 行，全部在 2016-12-01、全部沪市）—— 已修复**：

  | 代码 | 库内(错) | 已核实 | 通达信 .day | 同花顺 | 次日 pre_close |
  |---|---|---|---|---|---|
  | 600822.SH | 16.71 | **16.70** | 16.70 | 16.70 | 16.70 |
  | 603101.SH | 28.22 | **28.21** | 28.21 | 28.21 | 28.21 |
  | 603117.SH | 17.59 | **17.58** | 17.58 | 17.58 | 17.58 |
  | 603737.SH | 82.55 | **82.51** | 82.51 | 82.51 | 82.51 |

  三个独立来源一致（通达信本地 `.day`、同花顺 dump、baostock 自己的次日
  `pre_close`），且同源的 `open`/`high`/`low` 完全正确，只有 `close` 一个字段错。
  修复走 `util/dbutil.py` 的 `_KNOWN_DAILY_CLOSE_ANOMALIES`（按整条记录签名匹配，
  上游若修复或改值则规则自动失效），并对库内 4 行做了一次性 UPDATE ——
  STOCK_DAILY 是 UPSERT 写入，规则只防新数据写错，不会自愈旧行。
