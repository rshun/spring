...
tests/
  ...
  unit/
    ...
    test_validators.py
    test_tdx_logic.py
    test_akstock_logic.py
    test_bstock_logic.py
...
mock 对象：`ak.stock_info_bj_name_code`、`ak.stock_zh_a_hist`、`ak.stock_margin_sse`、`ak.stock_margin_szse`、`ak.stock_margin_detail_szse`、`ak.stock_margin_detail_sse`、`ak.stock_industry_clf_hist_sw`

**正常测试**
- `fetch_bj_stock_data`：列映射正确，返回 (df_info, df_basic)，df_info 含 exchange=BJ
- `fetch_stock_info(["BJ"])` 和 `fetch_stock_info(["all"])` 都触发 BJ 数据拉取
- `fetch_stock_info(["SH"])` → 返回空 DataFrame（不支持的交易所）
- `fetch_stock_data`：输出 df_daily 含 10 列，volume 已 ×100；df_basic 含 turnover_rate
- `_fetch_summary_sse`：列重命名正确，exchange_code=SH，trade_date 为 date 类型
- `_fetch_summary_szse`：多日汇总，过滤"融资融券"行，exchange_code=SZ
- `fetch_margin_detail`：深交所 symbol 只保留 `^[03]\d{5}$`，上交所只保留 `^6\d{5}$`
- `fetch_stock_industry_clf_hist_sw`：symbol zfill(6)，dropna，去重保留最后一条

**黑盒测试**
- `fetch_margin_detail` 输出列固定为 `_DETAIL_OUT_COLS` 中定义的 12 列
- `_fetch_summary_sse` / `_fetch_summary_szse` 输出列固定为 `_SUMMARY_OUT_COLS` 中定义的 10 列
- `fetch_stock_industry_clf_hist_sw` 返回列固定为 `[symbol, start_date, industry_code, update_time]`
- symbol 补零：`"1"` → `"000001"`，`"60001"` → `"060001"`

**异常测试**
- `fetch_bj_stock_data` ak 接口返回空 DataFrame → 返回 (空, 空)，打 warning，不抛异常
- `fetch_bj_stock_data` ak 接口抛异常 → 捕获，返回 (空, 空)
- `_fetch_summary_sse` ak 接口抛异常 → 返回空 DataFrame（含正确列），不向上抛
- `_fetch_summary_szse` 某日 ak 接口失败 → 跳过该日，继续处理其余日期
- `fetch_margin_detail` 两个交易所接口均失败 → 返回空 DataFrame（含正确列）
- `fetch_stock_industry_clf_hist_sw` ak 接口抛异常 → 返回空 DataFrame（含正确列）
- `fetch_stock_industry_clf_hist_sw` 返回缺字段 → 返回空 DataFrame，打 error log

---

### test_bstock_logic.py

mock 对象：`bs.login`、`bs.logout`、`bs.query_stock_basic`、`bs.query_history_k_data_plus`、`bs.query_adjust_factor`

**正常测试**
- `_is_not_logged_in("用户未登录，请重新登录")` → True
- `_is_not_logged_in("其他错误")` → False
- `_is_not_logged_in(None)` → False
- `_is_broken_pipe_error(BrokenPipeError())` → True
- `_is_broken_pipe_error(OSError())` with errno=32 → True
- `_is_broken_pipe_error(RuntimeError("Broken pipe"))` → True
- `_is_broken_pipe_error(ValueError("other"))` → False
- `_raise_for_query_error` 未登录消息 → 抛 `BaoNotLoggedInError`
- `_raise_for_query_error` broken pipe 消息 → 抛 `BrokenPipeError`
- `_raise_for_query_error` 其他错误 → 抛 `BaoQueryError`
- `fetch_stock_info` board 分类：300/301 开头 → GEM，688/689 → STAR，9 开头 → BJ，其余 → MAIN，指数 → INDEX
- `fetch_stock_data`：df_daily 含 10 列，price/volume NaN 填 0，PE/PB/换手保留 NaN
- `fetch_index_data`：输出 df_daily 含正确列，volume 转 int64

**黑盒测试**
- `fetch_stock_info` 输出 code 格式为 `600519.SH`（不是 `sh.600519`）
- `fetch_stock_data` 停牌日 price=0，volume=0，不报错
- `fetch_stock_info` 只保留 type=1（股票）和 type=2（指数），其他过滤掉

**异常测试**
- `fetch_stock_info` baostock 登录失败 → 返回空 DataFrame，不抛异常
- `fetch_stock_data` query 返回 error_code != "0" 且含"用户未登录" → 抛 `BaoNotLoggedInError`
- `fetch_stock_data` query 返回 error_code != "0" 且含"Broken pipe" → 抛 `BrokenPipeError`
- `fetch_stock_data` query 返回 error_code != "0" 其他错误 → 抛 `BaoQueryError`
- `fetch_stock_data` 返回空数据集 → 返回 (空 DataFrame, 空 DataFrame)
- `fetch_index_data` code 字段无法按 "." 分割 → 返回空 DataFrame
...
**正常测试**
- `fetch_bj_stock_data`：真实调用返回北交所股票列表，df_info 非空，exchange 列全为 BJ
- `fetch_stock_data`：拉取单只股票（如 000001）近 5 个交易日，df_daily 非空，必要列存在
- `fetch_margin_detail`：拉取指定日期沪深融资融券明细，返回 DataFrame 含正确列
- `fetch_stock_industry_clf_hist_sw`：返回申万行业历史，symbol 全为 6 位

**异常测试**
- 网络超时 / AkShare 接口报错 → 捕获异常返回空 DataFrame，不崩溃
...
**正常测试**
- `fetch_sync_calendar`：登录后拉取近 5 天日历，返回含 cal_date / is_open 的 DataFrame
- `fetch_stock_data`：拉取单只股票（如 sh.600519）近 5 天，df_daily 非空，列齐全
- `fetch_adjust_factors`：拉取单只股票复权因子，返回含 fore_factor / back_factor 的 DataFrame

**异常测试**
- baostock 登录失败（模拟错误 config）→ 返回空 DataFrame 或 None，不崩溃
