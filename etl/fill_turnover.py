...
回填换手率 turnover_rate
  ...
  流通股本来自 DAILY_BASIC.float_shares(由 fill_shares.py 依据 CAPITAL_DETAIL 回填)

  前置条件:
  1、先执行 import_daily.py, 确保日线数据已导入
  2、先执行 fill_shares.py, 确保 DAILY_BASIC.float_shares 已回填

  默认仅补齐 turnover_rate 为空的行(不覆盖); 传 -o/--overwrite 则区间内全部重算覆盖。
"""
import argparse
import duckdb
import logging
...
def main() -> None:
    ...
    logger.info(f"     覆盖模式: {'是' if args.overwrite else '否(仅补空)'}")
    logger.info("=" * 60)

    conn: duckdb.DuckDBPyConnection | None = None
    try:
        conn = dbutil.get_connection(is_read_only=False)
        updated = dbutil.fill_daily_basic_turnover(
            begin_date, end_date, codes, exchanges,
