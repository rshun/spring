...
CREATE TABLE IF NOT EXISTS SW_INDUSTRY (
    sw_code     VARCHAR(20) PRIMARY KEY,
    sw_name     VARCHAR(50) NOT NULL,
    sw_level    INTEGER NOT NULL CHECK (sw_level IN (1, 2, 3)),
    parent_code VARCHAR(20),
    updated_at  TIMESTAMP DEFAULT now()
);
COMMENT ON COLUMN SW_INDUSTRY.sw_code     IS '申万行业代码 (e.g. 801010.SI)';
COMMENT ON COLUMN SW_INDUSTRY.sw_name     IS '申万行业名称';
...
-- 股票申万行业历史映射（按生效日期追踪行业变更）
CREATE TABLE IF NOT EXISTS STOCK_SW_INDUSTRY (
    code           VARCHAR(20),
    effective_date DATE,
    sw_l1_code     VARCHAR(20),
    sw_l1_name     VARCHAR(50),
    sw_l2_code     VARCHAR(20),
    sw_l2_name     VARCHAR(50),
    sw_l3_code     VARCHAR(20),
    sw_l3_name     VARCHAR(50),
    entry_date     DATE,
    updated_at     TIMESTAMP DEFAULT now(),
    PRIMARY KEY (code, effective_date)
);
COMMENT ON COLUMN STOCK_SW_INDUSTRY.code           IS '股票代码';
COMMENT ON COLUMN STOCK_SW_INDUSTRY.effective_date IS '行业归属生效日期（首次入库或发生变更时更新）';
COMMENT ON COLUMN STOCK_SW_INDUSTRY.sw_l1_code     IS '申万一级行业代码';
COMMENT ON COLUMN STOCK_SW_INDUSTRY.sw_l1_name     IS '申万一级行业名称';
COMMENT ON COLUMN STOCK_SW_INDUSTRY.sw_l2_code     IS '申万二级行业代码';
COMMENT ON COLUMN STOCK_SW_INDUSTRY.sw_l2_name     IS '申万二级行业名称';
COMMENT ON COLUMN STOCK_SW_INDUSTRY.sw_l3_code     IS '申万三级行业代码';
COMMENT ON COLUMN STOCK_SW_INDUSTRY.sw_l3_name     IS '申万三级行业名称';
COMMENT ON COLUMN STOCK_SW_INDUSTRY.entry_date     IS '纳入申万行业时间（原始字段）';
COMMENT ON COLUMN STOCK_SW_INDUSTRY.updated_at     IS '记录写入时间';
