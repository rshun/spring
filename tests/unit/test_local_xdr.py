...
def test_micro_gate_is_restricted_to_pure_cash():
    ...
    闸门是为「派息小到被四舍五入抹平」这一种现象设的; 送转/配股无跳空属另一类
    (交易所当日不除权, 如 2005-06 股改对价), 归幅度上界与股改保护管。

    参数刻意取成能同时满足两个条件的平局点 —— 每 10 股送 0.005002501250626068 股,
    X = 10/(1+s) = 9.995, |X - 前收| = 0.005 恰在幅度上界上且恰好达到校正阈值。
    真实数据里落入该区间的只有 4 条配股价≈市价的配股(幅度 ~0.0009), 无法区分两种
    实现, 故以合成用例钉住契约。
    """
    conn = _seed_mem_conn()
    _insert_event(conn, "600000", "2026-03-11", bonus_share=0.005002501250626068)
    _insert_close(conn, "600000.SH", "2026-03-09", 10.0)
    # pre_close 与前收相等 -> 交易所当日未除权, 不触发跳空闸门
    _insert_close(conn, "600000.SH", "2026-03-11", 10.02, pre_close=10.0)

    out = _fetch(conn, [("600000", "SH", "2026-01-01", "2026-12-31", "L")])
    conn.close()

    # 保持名义口径, 不得被微额闸门压成 10.0/10.0 = 1.0
    assert out.iloc[0]["back_factor"] == pytest.approx(10.0 / 9.995)


def test_small_gap_on_high_priced_stock_is_corrected():
    """正例(2026-09-13 闸门改动): 高价股的小额分红跳空只有 0.078%, 旧的 0.1% 相对
    阈值把它挡在校正之外。实例 002261.SZ 2026-09-10: 前收 25.74, 交易所 25.72,
    gbbq 每股派 0.025 -> 名义 X=25.715。绝对阈值(差满半分即视为跳空)才放行。
    实测 2011 年以来 103 条比例错卡在这道闸门上。
    """
    conn = _seed_mem_conn()
    _insert_event(conn, "600000", "2026-03-11", dividend=0.25)     # 每股 0.025
    _insert_close(conn, "600000.SH", "2026-03-09", 25.74)
    # 跳空 0.02 元 = 0.078% < 0.1%, 旧阈值下不算跳空
    _insert_close(conn, "600000.SH", "2026-03-11", 25.80, pre_close=25.72)

    out = _fetch(conn, [("600000", "SH", "2026-01-01", "2026-12-31", "L")])
    conn.close()

    assert out.iloc[0]["back_factor"] == pytest.approx(25.74 / 25.72)
