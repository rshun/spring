...
lday / tdx 两个源是用 LAG(close) 现算的(见各自源码)。被它们覆盖过的股票, r_exch
恒等于 1, 本项对其的判别能力是**不对称**的:
  - 比例错、漏事件 —— 完全不可见(右边恒为 1, 无从比对);
  - 真实事件 —— 反而被报成「多事件」(因子跳了而 r_exch=1), 属误报。
所以对这类股票, 本项既不能证明其正确, 报出的「多事件」也不能当作缺陷。

已知落入盲区者(2026-09-13): 600387.SH / 000594.SZ —— 两只退市股的日线由 -s lday
补入(baostock 不覆盖北交所以外的退市股), 全部 pre_close 均等于前收; 实测 600387.SH
因此产生 8 条「多事件」误报。除此之外 2011 年以后无此类污染(2005-2007 有一批,
疑为股改而非污染)。
判定某股是否落入盲区: 该股 pre_close 与前一交易日收盘不等的行数为 0。
